...
const AIContentGenerator = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Store query in localStorage for admin dashboard
      const query = {
      ...
      try {
        ...
        query.response = response.content;
        setGeneratedContent(response.content);
      } catch (apiError) {
        // API failed, generate mock response
        const mockResponse = `This is a mock response for: "${prompt.trim()}". In a real implementation, this would be generated by ${selectedProvider}.`;
        query.response = mockResponse;
        setGeneratedContent(mockResponse);
      }

      // Store query in localStorage
      const existingQueries = JSON.parse(localStorage.getItem('ai_queries') || '[]');
      existingQueries.push(query);
      localStorage.setItem('ai_queries', JSON.stringify(existingQueries));
